...
exports.create = (req, res) => {
  ...
  const coupon = new Coupon({
    ...
    applicableForSize: Number(sizeToSave), 
    isActive: true
  });
...
exports.verifyCoupon = async (req, res) => {
  try {
    ...
    // 🚨 NAYA CHECK: Corrupted/purane coupons ke liye
    let cSize = coupon.applicableForSize;
    if (cSize === undefined || cSize === null) {
      cSize = coupon.applicableforSize; 
    }
    if (cSize === undefined || cSize === null || isNaN(cSize)) {
      return res.status(400).json({ success: false, message: "Corrupted coupon data! Admin needs to recreate it." });
    }

    // 0 matlab All Accounts, toh pass hone do
...
exports.update = (req, res) => {
  ...
  Coupon.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then(data => {
      if (!data) res.status(404).send({ message: `Cannot update Coupon with id=${id}. Maybe Coupon was not found!` });
...
exports.delete = (req, res) => {
  ...
  Coupon.findByIdAndRemove(id, { useFindAndModify: false })
    .then(data => {
      if (!data) res.status(404).send({ message: `Cannot delete Coupon with id=${id}. Maybe Coupon was not found!` });
